    ...
    <>
      {messages.map((message, index) => (
        <div key={index} className="flex items-start gap-4 p-4">
          ...
          <div>
            <h5>{message.isUser ? "Som" : "Novus, AI Recruiter"}</h5>
...
/* export default function ChatView({ messages }) {
      ...
      {messages.map((message, index) => (
        <div key={index} className="flex items-start gap-4 p-4">
          ...
          <div className="w-full">
            <h5>{message.isUser ? "Som" : "Novus, AI Recruiter"}</h5>
            <p className="text-gray-500 whitespace-pre-line">{message.text}</p>
          </div>
        </div>
      ))}
    </>
  ...
  /* export default function ChatView({ messages }) {
    return (
      <div className="chat-view-container overflow-y-auto" style={{ maxHeight: '400px' }}>
        {messages.map((message, index) => (
          <div key={index} className="flex items-start gap-4 p-4">
            <div className="h-10 bg-white w-10 text-center flex-none rounded-full">
              {message.isUser ? (
        ...
        <div className="chat-view-container">
          {messages.map((message, index) => (
            <div key={index} className="flex items-start gap-4 p-4">
              ...
              <div className="w-full">
                <h5>{message.isUser ? "Som" : "Novus, AI Recruiter"}</h5>
                <Markdown className="text-gray-500 whitespace-pre-line">
                  {message.text}
                </Markdown>
              </div>
            </div>
